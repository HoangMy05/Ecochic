...
jQuery(function ($) {
  ...
  function super_mart_store_photography_slider_animation(selector) {
  ...
  $(document).on('bkSlideinit', function () {
    var observer = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          super_mart_store_photography_slider_animation('.swiper-slide-active');
          return;
        }
      });
    });
    observer.observe(document.querySelector('.swiper-wrapper'));
  }); // super_mart_store_photography_slider_animation('.swiper-slide:first-child');
