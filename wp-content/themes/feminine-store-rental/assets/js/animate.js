...
jQuery(function ($) {
  ...
  function feminine_store_rental_photography_slider_animation(selector) {
  ...
  $(document).on('bkSlideinit', function () {
    var observer = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          feminine_store_rental_photography_slider_animation('.swiper-slide-active');
          return;
        }
      });
    });
    observer.observe(document.querySelector('.swiper-wrapper'));
  }); // feminine_store_rental_photography_slider_animation('.swiper-slide:first-child');
