 ...
 * @return array
 */
function swimwearstore_body_classes( $classes ) {
...
add_filter( 'body_class', 'swimwearstore_body_classes' );
...
if (!function_exists('swimwearstore_str_replace_assoc')) {

    /**
     * swimwearstore_str_replace_assoc
     * @param  array $replace
     * @param  array $subject
     * @return array
     */
    function swimwearstore_str_replace_assoc(array $replace, $subject) {
 ...
 */
function swimwearstore_add_to_cart_fragment( $fragments ) {
	...
	<a class="nav-link cart-icon-wrap header-cart" href="javascript:void(0);" role="button" data-bs-toggle="collapse" data-bs-target="#Cart">
		...
		?>	
			<span><?php echo esc_html__('0','swimwear-store'); ?></span>
		<?php
		}
		?>
	</a>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'swimwearstore_add_to_cart_fragment' );
...
if ( ! function_exists( 'swimwearstore_breadcrumb_title' ) ) {
	function swimwearstore_breadcrumb_title() {
		...
		elseif ( is_day() ) : 
			...
			printf( __( 'Daily Archives: %s', 'swimwear-store' ), get_the_date() );
		
		elseif ( is_month() ) :
		
			printf( __( 'Monthly Archives: %s', 'swimwear-store' ), (get_the_date( 'F Y' ) ));
			
		elseif ( is_year() ) :
		
			printf( __( 'Yearly Archives: %s', 'swimwear-store' ), (get_the_date( 'Y' ) ) );
			
		elseif ( is_category() ) :
		
			printf( __( 'Category Archives: %s', 'swimwear-store' ), (single_cat_title( '', false ) ));

		elseif ( is_tag() ) :
		
			printf( __( 'Tag Archives: %s', 'swimwear-store' ), (single_tag_title( '', false ) ));
			
		elseif ( is_404() ) :

			printf( __( 'Error 404', 'swimwear-store' ));
			
		elseif ( is_author() ) :
		
			printf( __( 'Author: %s', 'swimwear-store' ), (get_the_author( '', false ) ));		
...
function swimwearstore_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','swimwear-store'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','swimwear-store'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','swimwear-store'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
    ...
    if ( is_category() ) 
	{
		...
		echo $before . esc_html__('Archive by category','swimwear-store').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for','swimwear-store').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','swimwear-store').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','swimwear-store').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','swimwear-store'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','swimwear-store') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
...
add_action( 'wp_ajax_swimwearstore_dismissed_notice_handler', 'swimwearstore_ajax_notice_handler' );

/**
 * AJAX handler to store the state of dismissible notices.
 */
function swimwearstore_ajax_notice_handler() {
    if ( isset( $_POST['type'] ) ) {
...
add_action( 'wp_ajax_install_act_plugin', 'swimwearstore_admin_install_plugin' );

function swimwearstore_admin_install_plugin() {
    /**
     * Install Plugin.
     */
